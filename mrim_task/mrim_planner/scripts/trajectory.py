...
class TrajectoryUtils():
    ...
    def interpolateHeading(self, waypoints):
        ...
        while idx != (len(waypoints) - 1):
            ...
            for i in range(1, len(subtraj) - 1):
                subtraj_0 = subtraj[i - 1]
                subtraj_1 = subtraj[i]
                ratio = self.getLength([subtraj_0, subtraj_1]) / subtraj_len
                ...
                hdg_interp = wrapAngle(subtraj_0.heading + ratio*d_hdg)
    ...
    def sampleTrajectoryThroughWaypoints(self, trajectory, with_stops, smooth_path, smoothing_la_dist, smoothing_sampling_step, velocity_limits, acceleration_limits):
        ...
        if with_stops:
            ...
            ts_samples = np.arange(0.0, toppra_trajectory.duration, sampling_step)
            samples = toppra_trajectory.eval(ts_samples)
    ...
    def computeTimeParametrization(self, waypoints, v_lims, a_lims):
        ...
        gridpoints = np.linspace(0, path.duration, 100000)
