...
class ProblemLoader:
    ...
    def loadWorld(self, filepath):
        ...
        min_height, max_height = float(data_loaded['safety_area']['min_height']), float(data_loaded['safety_area']['max_height'])
        lateral_corners = []
        for idx in range(0, len(data_loaded['safety_area']['safety_area']), 2):
            corner = ROSPoint()
            corner.x = data_loaded['safety_area']['safety_area'][idx]
            corner.y = data_loaded['safety_area']['safety_area'][idx+1]
            corner.z = 0
